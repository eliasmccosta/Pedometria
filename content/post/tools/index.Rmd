...
image:
  ...
  placement: 3
math: true
title: Ferramentas pedometricas
subtitle: 'Ferramentas pedometricas para análise de pedologia quantitativa'
summary: ''
...
## Análise de Dissimilaridade (Dendogramas)
Os dendogramas são uma maneira conveniente de descrever a dissimilaridade entre pares (*pair-wise*), comumente associada ao tópico da análise de cluster.
O cálculo da dissimilaridade entre perfis é realizado usando a função `profile_compare()`. Os valores de dissimilaridade dependem da seleção de atributos (por exemplo, argila, CEC, pH etc.), parâmetro opcional de ponderação em profundidade `(k)` e uma profundidade máxima de avaliação `(max_d)`. Maiores informações dobre análise de dissimilaridade pode ser encotrado em @Beaudette2013 e com exemplos de aplicação em @Pinheiro2016 e @Pinheiro2018.
...
# Parte III
Nessa parte é usado outra base de dados do `soilDB`, e aqui além de plotes que marcam os horizontes diagnósticos de alguns perfis e feitos alguns plots que relacionam o perfil e a paisagem. Maiores comentários estam na linha de código

```{r, warning=FALSE, message=FALSE}
# Carregando outros dados de solos
data("loafercreek")
...
# Adicionar marcação para alguns horizontes diagnósticos
...
crs.utm <- CRS('+proj=utm +zone=10 +datum=NAD83') #reprojetando para utm metros
par(mar=c(4.5,4,1,1))
plotTransect(x, grad.var.name = 'elev_field', crs = crs.utm)
plotTransect(x, grad.var.name='elev_field', crs=crs.utm, grad.axis.title='Elevação (m)', 
